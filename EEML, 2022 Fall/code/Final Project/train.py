...
def main():
	X = np.load('X_train.npy').astype(np.float)
	y = np.load('y_train.npy').astype(np.float)
#	print(X, X.shape)
#	print(y, y.shape)
	y_min, y_max = 10000, 1000000
#	print(y_min, y_max)
...
#	rs = 87
	...
	for i, mtype in enumerate(model_types):
		...
		sub_model[i][1].fit(X_train, np.log(y_train.squeeze()))
		print('Model', mtype)
	...
	print(weights)
	model = VotingRegressor(sub_model, weights = weights, n_jobs = 8, verbose = 1)
...
#	print(model.best_params_)
	...
	test = np.load('X_test.npy').astype(np.float)
	add_dim(test, len(test[0]) * dim)
	test = np.hstack((test, np.ones((len(test), 1))))
	predict = np.power(np.e, model.predict(test))
	predict = np.clip(predict, y_min, y_max)
